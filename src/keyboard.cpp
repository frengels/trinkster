    ...
    : server_{serv}, device_{device}
{
    ...
    modifiers_.notify = handle_modifiers;
    wl_signal_add(&device_->keyboard->events.modifiers, &modifiers_);
    key_.notify = handle_key;
    wl_signal_add(&device_->keyboard->events.key, &key_);

    wlr_seat_set_keyboard(serv->seat(), device_);
}

void keyboard::handle_modifiers(wl_listener* listener, void*)
{
    keyboard* self = wl_container_of(listener, self, modifiers_);

    wlr_seat_set_keyboard(self->server_->seat(), self->device_);
    wlr_seat_keyboard_notify_modifiers(self->server_->seat(),
                                       &self->device_->keyboard->modifiers);
}

void keyboard::handle_key(wl_listener* listener, void* data)
{
    ...
    bool     handled   = false;
    uint32_t modifiers = wlr_keyboard_get_modifiers(self->device_->keyboard);
